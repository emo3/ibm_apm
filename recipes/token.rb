...
xml_resp.xpath('//variable').each do |node|
  next unless node.run_state['name'] == 'client.secret.apmui'
  value = node['value']
  break
end
...
# puts ("value=#{value}")
output = Mixlib::ShellOut.new("/sfcots/apps/apm/kafka/bin/java -cp #{classpath} com.ibm.ws.security.util.PasswordDecoder #{value}")
output.run_command
decodedpw = output.stdout(/decoded password == "(.*)"/)[1]
